import { Meta, Subtitle } from '@storybook/addon-docs/blocks';
import { Box, Image } from './components';
import duplicateToYourDrafts from './imgs/duplicate-to-your-drafts.png';
import duplicatedFile from './imgs/duplicated-file.png';
import backToFiles from './imgs/back-to-files.png';
import drafts from './imgs/drafts.png';

<Meta title="Foundations/Get Started/Design" />

# Getting Started — Design

The Radius Design Kit is built and maintained in Figma. It includes a set of foundational components that you can build upon to start your own design system.

### Set up

1. Sign up for [Figma](https://www.figma.com/). If you already have an account, continue to sign-in with your credentials. (If you're a Rangle employee, reach out to the administrator for access to the organization)

2. You may access Figma through the browser or download the desktop application. To download the desktop app, click on the link for your operating system:

   - [Desktop App for macOS](https://www.figma.com/download/desktop/mac/)
   - [Desktop App for Windows](https://www.figma.com/download/desktop/win/)

3. The Radius Design Kit is view only but it can be copied and accessed here: [https://www.figma.com/file/RqENxZWAzGiEWM7COch1Sc/Radius-Design-System](https://www.figma.com/file/RqENxZWAzGiEWM7COch1Sc/Radius-Design-System)

4. To make a copy, select _Duplicate to your Drafts_ from the file dropdown.

   <Image
     src={duplicateToYourDrafts}
     maxWidth={768}
     display="block"
     border="1px solid #757575"
     style={{ marginBottom: '2rem' }}
   />

5. Click the call to action to open your duplicated file.

   <Image
     src={duplicatedFile}
     maxWidth={768}
     display="block"
     border="1px solid #757575"
     style={{ marginBottom: '2rem' }}
   />

6. Once you complete these steps, you will have full access to edit the copied Radius Design Kit. To go back to your files, click on the main menu.

   <Image
     src={backToFiles}
     maxWidth={768}
     display="block"
     border="1px solid #757575"
     style={{ marginBottom: '2rem' }}
   />

7. You can find your copied file in _Drafts_, as well as in your recently viewed files.
   <Image
     src={drafts}
     maxWidth={768}
     display="block"
     border="1px solid #757575"
     style={{ marginBottom: '2rem' }}
   />

### Resources for Figma

- Best Practices: [https://www.figma.com/best-practices/](https://www.figma.com/best-practices/)
- Templates: [https://www.figma.com/templates/](https://www.figma.com/templates/)
- Video tutorials: [https://www.youtube.com/channel/UCQsVmhSa4X-G3lHlUtejzLA](https://www.youtube.com/channel/UCQsVmhSa4X-G3lHlUtejzLA)
