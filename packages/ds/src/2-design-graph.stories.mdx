...
<Meta title="Foundations/Variants" />
...
They define the visual characteristics of a brand or a product such as typography, colors, icons, and spacing. They make it easier to maintain a scalable, consistent system for UI development. RDS maintains all design tokens in a `theme.ts` file.
...
Now, this component will have style props available to set foreground and background color and margin & padding value.
 ...
 */
<Box color="textEditorial" bg="background.1" p={3}>
  Tomato
</Box>
```
...
[Styled System variants](https://styled-system.com/variants) allow you to apply a set of visual styles to a component using a single prop. These variants also Variants also map to your design tokens, defined in `theme.ts`.
...
The Text component can now use the `variant` prop to change between lead, body and label styles.
