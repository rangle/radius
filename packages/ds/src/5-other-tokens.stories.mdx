      ...
      {spaceScale.map((v, idx) => (
        <Flex key={v + idx} mb={4} alignItems="center">
          <Text variant="body" minWidth={4} mb={0}>
            {idx} <Text as="span" variant="body" color="text.secondary" mb={0}>{`(${v}px)`}</Text>
          </Text>
          <Box bg="ui.primary" height={2} style={{ width: v }} />
        </Flex>
      ))}
    </Box>
  </Story>
  ...
  <Story name="breakpoints">
    <Box>
      <Flex alignItems="baseline" mb={4}>
        <Text variant="label" color="text.secondary" minWidth={4} mb={0} mr={3}>
          Small Breakpoint
        </Text>
        <Heading variant="heading-5" mb={0}>
          &lt; 40em
        </Heading>
      </Flex>
      <Flex alignItems="baseline" mb={4}>
