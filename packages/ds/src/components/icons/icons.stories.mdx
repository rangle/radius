...
When used for **non-decorative** purposes, it must have a proper text alternative via `aria-label` for screen readers.

<Preview withSource={SourceState.OPEN}>
  <Story name="as non-decorative">
    <Icons.Loader aria-label="waiting for content to load" />
  </Story>
</Preview>
