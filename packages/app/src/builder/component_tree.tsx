...
export const ComponentTree = ({onSelect, state}: {
    state: ControlState; onSelect: (val: ControlState) => void;
}) => {
    ...
    return (
        <Stack axis="vertical">
            ...
            <Box pl={3}>
                {isControlState(contents) ? (
                    <ComponentTree state={contents} onSelect={onSelect} />
                ) : Array.isArray(contents) ? (
                    contents.map((c, i) => (
                        <ComponentTree key={i} state={c} onSelect={onSelect} />
                    ))
                ) : (
                    Object.keys(contents).map((k) => {
                        const labeledContent = contents[k];
                        const items = Array.isArray(labeledContent)
                            ? labeledContent
                            : [labeledContent];
                        return (
                            <Stack axis="vertical" space={1} key={k}>
                                <Label>{k}</Label>
                                {items.map((item, i) => (
                                    <ComponentTree
                                        key={i}
